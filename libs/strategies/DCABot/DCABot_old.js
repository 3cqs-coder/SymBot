...
async function test() {
    ...
    const exe = new ccxt.pro["binance"]({
        // urls: {
    ...
    let usdtBal = bala.USDT;
    console.log("balance USDT", bala.USDT);
}
...
async function start(dataObj) {
  ...
  console.log("firstOrderPrice", firstOrderPrice);
      ...
      if (deal.isStart == 0) {
        ...
        if (baseOrder.type == "MARKET") {
          ...
          if (price <= baseOrder.price) {
            ...
            // if (!config.sandBox) {
            //   const priceFiltered = await filterPrice(exchange, pair, baseOrder.price);
            //   console.log("priceFiltered",priceFiltered);

            //   const buy = await  buyOrderlimit(
            //     exchange,
            //     dealId,
            //     pair,
            //     baseOrder.qty,
            ...
            //   console.log("buy",buy.data.status);
...
console.log("cctx", ccxt.exchanges);
...
const buyOrderl = async (exchange, dealId, pair, qty, price) => {
  ...
  try {
    ...
    if (order.status === 'open' || order.status === "OPEN") {
      ...
      let result = await watchDog1(order.id, order.price, order.symbol,exchange);
...
const buyOrderlimit = async (exchange, dealId, pair, qty, price) => {
  let msg;
  let order;
  let isErr;
  let success;
  try {
    ...
    console.log(dealId, pair, "0.009", price);
    order = await exchange.createOrder(pair, "limit", "buy", "0.009", price);
    console.log("xsdsedfse",order);
    success = true;
    msg = "BUY SUCCESS";
  } catch (e) {
    isErr = e;
...
async function connectExchange(configObj) {
  ...
  try {
    ...
    if (
      ...
      // // const a1=await exchange.loadMarkets();
      // let bala1 = await exchange.fetchBalance();
      // console.log("balance USDT-----------",bala1.USDT);
      // console.log(config);
      // console.log(config.exchange);
      const exchange = new ccxt.pro["binance"]({
		    apiKey: 'AZxv3kIMvl1RNgsWgrbNE8o6E1a0rHGMJnMYrh5q1DJp7oWAoUacccYCfw0pf6m5',
        secret: 'Kx5H4C8yrwMFhtGqeL2sZaGtgxQng8mT5G4ZEFoWK6Pwkj9wjJDCrfrYFOhqIqCK'
      });
      exchange.setSandboxMode(true); //it will activate Test mode
      await exchange.loadMarkets();
      let bala = await exchange.fetchBalance();
      console.log("balance USDT++++++++++++", bala.USDT,bala.ETH);
