...
function initApp() {
	...
	// X-RateLimit-* headers). No-op for sessions, the owner, and keys with no limit set. Also applies
	// a small fixed per-identity cap to the disruptive /system/restore, /system/update, /system/rollback,
	// /system/shutdown endpoints regardless of principal type, since the per-key limit above is a no-op
	// for sessions/the owner (see AuthMiddleware.systemControlLimit).
